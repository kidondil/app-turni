...
Dal Profilo, l'amministratore può inoltre aprire **Importa turni del mese** e caricare un CSV con le colonne base `Data;Turno;Autista;Capoturno;Soccorritore`. Le colonne facoltative `Data fine;Assente;Tipo assenza` permettono di importare anche la Malattia come intervallo: per esempio `01/09/2026;30/09/2026;;;;;Cagnin Giorgia;Malattia` nel modello completo. L'app controlla nomi, gruppi, duplicati, assenze e smontanti, mostra l'anteprima giorno per giorno e richiede una conferma. Gli spazi interni ai nomi non incidono sul riconoscimento, quindi `Gianfranco` e `Gian Franco` sono equivalenti. Nell'importazione sono autorizzate anche le sostituzioni ricorrenti di Andrea Caddeo come Soccorritore e Lucia Murtas come Capoturno; per ogni altro operatore il controllo del gruppo resta invariato. La modalità normale aggiorna soltanto squadre e malattie presenti nel file; l'opzione **Sostituisci tutto il mese** sostituisce anche le malattie precedentemente importate per quel mese, conservando quelle registrate manualmente.

Le date sono mostrate nel formato italiano `GG/MM/AAAA`. Ferie, Permesso e Malattia si inseriscono digitando il periodo oppure selezionando il primo e l'ultimo giorno dal calendario. La Malattia può essere registrata dall'utente interessato o da un amministratore, diventa subito attiva, non produce ore e impedisce nuove assegnazioni nel periodo; un amministratore può scegliere la persona direttamente nella schermata. Nel calendario è indicata in viola con la sigla `Mal` e nel dettaglio del giorno vengono mostrati i nomi interessati. Ogni operatore può annullare una propria assenza ancora attiva, purché il periodo non sia concluso. Può inoltre annullare uno scambio inviato finché è ancora in attesa; le richieste restano nello storico con lo stato **Annullata** o **Annullato**.
